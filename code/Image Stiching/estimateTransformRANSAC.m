function [P,S,V,q,A_ransac]= estimateTransformRANSAC(img1_points,img2_points)
    %% Intialization
    ...
    N_iter = 500 %round(log10(1-p)/log10(1-(1-e)^k)); % no. of iterations
    %distThreshold = sqrt(5.99*sigma) %sigma = expected uncertainty
    %inlierThreshold = (1-e)*N_pts
    ...
    for i = 1:N_iter
        ...
        randIndexes = randi(N_pts,k,1);
        im1pts = img1_points(randIndexes,:);
        im2pts = img2_points(randIndexes,:);
        
        %% Homography Matrix estimation
        
        [P,S,V,q,A] = estimateTransform(im1pts,im2pts);
        ...
        if nMatchedInliers > maxMatchedInliers
            %eF = (1-nInliersF/N_pts);
            %eB = (1-nInliersB/N_pts);
            %e = nFB*eF + (1-nFB)*eB;
    ...
    [P,S,V,q,A_ransac] = estimateTransform(im1InlierCorrPts,im2InlierCorrPts);
end